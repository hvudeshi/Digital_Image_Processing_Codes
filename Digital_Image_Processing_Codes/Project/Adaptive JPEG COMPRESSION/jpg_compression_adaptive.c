...
double* getZigZagMatrix( double **arr, int n, int m)
{
    int row = 0, col = 0;
    ...
    int array_size=m*n;
    double* w;
    w = (double *)malloc(array_size * sizeof(double));
    int array_counter=0;
...
void main()
{
   ...
   //Input Image
    ...
    double **testV = (double **)malloc(64 * sizeof(double *));
    for (i=0; i<64; i++)
         testV[i] = (double *)malloc(11 * sizeof(double)); 

    double *NQ;
    NQ = (double *)malloc(64 * sizeof(double));
    ...
    for(scan=0;scan<nscan;scan+=8)
    {
        for(pix=0;pix<npix;pix+=8)
        {
            ...
            //Storing Q_Matrix in ZigZag manner
            init_Q_matrix = getZigZagMatrix(q_final,8,8);

            //Storing 11 Q_matrices
            for(i=0;i<64;i++)
            {
                testV[i][0] = init_Q_matrix[i]-5;
                testV[i][1] = init_Q_matrix[i]-4;
                testV[i][2] = init_Q_matrix[i]-3;
                testV[i][3] = init_Q_matrix[i]-2;
                testV[i][4] = init_Q_matrix[i]-1;
                testV[i][5] = init_Q_matrix[i];
                testV[i][6] = init_Q_matrix[i]+5;
                testV[i][7] = init_Q_matrix[i]+4;
                testV[i][8] = init_Q_matrix[i]+3;
                testV[i][9] = init_Q_matrix[i]+2;
                testV[i][10] = init_Q_matrix[i]+1;

            }


            double max = (double)-200000,min=(double)200000;
            double *AMSE1;
            double *AMSE2;
            int ind1=0,ind2=0;
    ...
    for(scan=0;scan<nscan;scan+=8)
    {
        for(pix=0;pix<npix;pix+=8)
        {
          ...
          for (i = 0; i < 64; ++i)
          {
            ...
            block[index[i]/8][index[i]%8]*=NQ[index[i]];
          }
